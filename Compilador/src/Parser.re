let parse_constant = tokenList: Ast.exp =>{
  let [token, ...remainingTokens] = tokenList; /*Sacamos siguiente token que es el número*/
  ...
  Ast.Const(number) /*Devolvemos el nodo constant*/
};
...
let parse_statement = tokenList: Ast.statement => {
  ...
  /*El siguiente elemento es el número, por lo que se pasa al parse_constant*/
  let constant = parse_constant(remainingTokens);

  let [token, ...remainingTokens] = remainingTokens; /*Sacamos siguiente token que debe de ser el return ";"*/
  token |> Token.identificador |> Js.log;
...
let parse_function = tokenList: Ast.fun_decl => {
  ...
  let return = parse_statement(remainingTokens); 
